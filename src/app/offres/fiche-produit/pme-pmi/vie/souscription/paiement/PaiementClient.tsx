...
export default function PaiementClient() {
  ...
  const canPay = Boolean(
    (isMobileMethod && mobilePhone.trim().length >= 8) || isCard || isVirement
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent>
          ...
          {isVirement && (
            <div className="mt-4 grid gap-3">
              ...
              <a href="/docs/rib-sanlam.pdf" download className={cn(buttonVariants(), "w-fit")}>
      ...
      <div className="flex items-center justify-between gap-4">
        <Link
          href="/offres/fiche-produit/pme-pmi/vie/souscription/documents"
        ...
        <button
          ...
          onClick={() => {
            if (isCard) {
              const url = `https://pay.sanlam.test/link/${encodeURIComponent(
                cotation
              )}-${Date.now()}`;
              setLinkUrl(url);
              setLinkSent(true);
